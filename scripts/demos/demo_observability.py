...
async def demo_observability():
    ...
    with observability_context(
        agent_name="DemoAgent", step_id="step-1", pipeline_id="demo-pipeline"
    ):
    ...
    metrics.record_agent_execution(
        "synthesis", False, 45.2, tokens_used=75, error_type="ValidationError"
    )
    ...
    for component_name, health in health_results.items():
        status_icon = (
            ...
            else "⚠️" if health.status.value == "degraded" else "❌"
        )
        print(
            f"{status_icon} {component_name}: {health.status.value.upper()} - {health.message}"
        )
    ...
    try:
        ...
        print(
            f"Average Duration: {performance_summary['average_execution_time_ms']:.2f}ms"
        )
        print(
            "System diagnostics available - health checks and metrics collection working!"
        )
    ...
    with observability_context(pipeline_id="monitoring-demo"):
        ...
        for i in range(3):
            with observability_context(
                agent_name=f"Agent-{i+1}", step_id=f"step-{i+1}"
            ):
                ...
                if success:
                    logger.info(
                        f"Agent-{i+1} completed successfully", duration_ms=duration_ms
                    )
                else:
                    logger.error(
                        f"Agent-{i+1} failed",
