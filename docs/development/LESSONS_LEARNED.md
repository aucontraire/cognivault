...
- **Dormant code isn't just unused — it's unverified.** `TopicRepository.find_similar_by_embedding` shipped ~7 months before feature 002 with a `:embedding::vector` bind that SQLAlchemy's `text()` misparses (the `::` cast collides with `:param` syntax), sending a stray `:` to Postgres. It had **zero call sites**, so no test and no run ever executed it — the syntax error sat latent, looking "ready," until feature 002's semantic search became its first-ever caller and surfaced it on the first invocation. A method that compiles and reads correctly but is never exercised is a liability, not an asset. Treat unreferenced code as unverified, not done — and be suspicious of "we'll wire it up later" plumbing that ships green. (Fixed with `CAST(:embedding AS vector)`; caught by the US3 similarity test, the first code to ever call it.)
- **Fixtures validate the mechanism; production data reveals the corpus.** Feature 002's semantic retrieval scored 16/16 on rich-description fixtures but returns **0 results on the real corpus** at the production `0.3` similarity threshold. Controlled measurement (pre-registering "related" labels *before* computing similarities) showed the topic **name** dominates: a specific name + boilerplate description already clears 0.3, a vague name (`architectures`, `testing` — what `TopicManager` actually emits) embeds near the noise floor (~0.13) and never surfaces. So the gap is a topic-extraction *quality* problem, not a retrieval-code defect — and the "proven end-to-end" claim was really "proven at threshold 0," a more permissive path than production. Lesson: evaluate retrieval on **real data** (with pre-registered relevance labels and the production threshold) before declaring the loop closed. Full trail: `.internal-docs/investigations/2026-07-05-semantic-retrieval-topic-quality.md`.
