...
class TestOpenAISchemaGeneration:
        ...
        def check_refs(obj: Any, path: str = "root") -> None:
            if isinstance(obj, dict):
                if "$ref" in obj and len(obj) > 1:
                    ...
                    errors.append(
                        f"$ref at {path} has additional keywords: {other_keys}"
                    )
            ...
            for def_name, def_schema in schema["$defs"].items():
                if "properties" in def_schema:
                    ...
                    if def_props != def_required:
                        ...
                        if missing:
                            errors.append(
                                f"Nested model {def_name} missing from required: {missing}"
                            )
    ...
    def test_custom_model_with_optional_fields(self, service: LangChainService) -> None:
        ...
        class TestModel(BaseModel):
            ...
            optional_with_default: Optional[int] = Field(
                10, description="Optional with default"
            )
