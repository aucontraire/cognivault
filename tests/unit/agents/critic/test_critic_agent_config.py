...
class TestCriticAgentConfig:
    ...
    def test_critic_agent_backward_compatibility(self) -> None:
    ...
    def test_critic_agent_with_config_parameter(self) -> None:
        """Test that CriticAgent accepts config parameter."""
        mock_llm = Mock(spec=LLMInterface)

        # Create custom config
        custom_config = CriticConfigFactory.deep_analysis(
            confidence_reporting=False, scoring_criteria=["accuracy", "thoroughness"]
        )
    ...
    def test_prompt_composer_integration(
        ...
        config = CriticConfigFactory.deep_analysis()
    ...
    def test_update_config_method(self, mock_prompt_composer_class: object) -> None:
        ...
        # Update config
        new_config = CriticConfigFactory.comprehensive_analysis(
    ...
    async def test_run_method_uses_configurable_prompt(
        ...
        context = AgentContextFactory.with_agent_outputs(
            query="Original query", refiner="Refined query output"
        )
    ...
    @pytest.mark.asyncio
    async def test_run_method_backward_compatibility(self) -> None:
        ...
        # Create context
        context = AgentContextFactory.with_agent_outputs(
            query="Test query", refiner="Refined test query"
        )
    ...
    @pytest.mark.asyncio
    async def test_run_method_handles_missing_refiner_output(self) -> None:
        """Test that run method handles missing refiner output gracefully."""
        mock_llm = Mock(spec=LLMInterface)
        agent = CriticAgent(llm=mock_llm)

        # Create context without refiner output
        context = AgentContextFactory.basic(query="Test query")
